...
SELECT c.first_name, c.last_name, SUM(payment.amount) AS dollars_spent
FROM customer c
...
SELECT s.store_id, a.address,COUNT(r.rental_id) AS total_number_of_rentals, SUM(p.amount), AVG(p.amount)
FROM store s
